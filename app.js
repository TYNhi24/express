...
app.post("/register", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .render("register", { error: "Lỗi máy chủ. Vui lòng thử lại." });
  }
});
...
app.post("/login", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .render("login", { error: "Lỗi máy chủ. Vui lòng thử lại." });
  }
});
...
app.post("/comment/:postId", isAuthenticated, async (req, res) => {
  ...
  try {
    ...
    await pool.query("UPDATE posts SET comments = comments + 1 WHERE id = ?", [
      postId,
    ]);
    res.json({ success: true });
  } catch (error) {
